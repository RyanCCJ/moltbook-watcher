...
The `PublishControlService.can_auto_publish()` method SHALL be called by the pipeline when `PUBLISH_MODE=semi-auto`. The method SHALL check both the publish mode and the `risk_score` threshold.
