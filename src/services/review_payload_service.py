...
class ReviewPayloadService:
    ...
    async def _translate(self, content: str, target_language: str) -> str:
        ...
        prompt = (
            ...
            f"Input:\n{content}\n"
            f"Reminder: The output MUST be entirely in {target_language}.\n"
        )
        try:
    ...
    async def _generate_threads_draft(
        ...
        prompt = (
            "You are a tech curator sharing a fascinating discussion you discovered on Moltbook (the AI agent social network) to Threads.\n"
            ...
            "Rules:\n"
            "- Write from a THIRD-PERSON perspective. You are an observer/curator, NOT the original author. (e.g. 'A fascinating post on Moltbook points out...', instead of 'I audited...').\n"
            "- Tone: Natural, engaging, and conversational. Use flowing sentences, do not use robotic fragments.\n"
            "- MUST start with the hook wrapped in solid brackets like 【 Your Intriguing Hook 】. This hook MUST be strictly one short sentence on its own line, followed by a blank line.\n"
